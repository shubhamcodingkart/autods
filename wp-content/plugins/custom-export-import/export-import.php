...
/*
  Plugin Name: Update Product from a CSV file
 ...
 */
class Custom_Dokan_Product_Importer {
    ...
    public function __construct() {
        ...
        add_action( 'wp_ajax_woocommerce_do_ajax_product_import_custom', array( $this, 'do_ajax_product_import' ) );
    ...
    function handle_step_submission() {
        ...
        $import_controller = new Custom_WC_Product_CSV_Importer_Controller();
    ...
    public static function init() {
        ...
        if ( !$instance ) {
            $instance = new Custom_Dokan_Product_Importer();
        }
    ...
    public function enqueue_scripts() {
        ...
        if ( dokan_is_seller_dashboard()  && isset( $wp->query_vars['update_products'] )) {
        ...
        wp_register_script( 'custom-wc-product-import', home_url() . '/wp-content/plugins/custom-export-import/assets/js/wc-product-import-2.js', array( 'jquery' ), WC_VERSION );
    }
    ...
    function add_endpoint( $query_var ) {
        $query_var['update_products'] = 'update_products';
    ...
    public function add_importer_page( $urls ) {
        ...
        if ( dokan_is_seller_enabled( get_current_user_id() ) ) {
            ...
            if ( $installed_version >= '2.4' ) {
                $urls['update_products'] = array(
                    'title' => __( 'update_products', 'dokan' ),
                    'icon'  => '<i class="fa fa-wrench"></i>',
                    'url'   => dokan_get_navigation_url( 'update_products' ),
                    'pos'   => 192
                );
            } else {
                $urls['update_products'] = array(
    ...
    public function dashboard_template( $template_part ) {
        print_r($template_part);
        echo dokan_get_template_part( 'export-import/template_importer', '', array( 'is_export_import' => true ) );
        return;
    }
    ...
    **/
    ...
    function load_tools_template_from_plugin( $query_vars ) {
        ...
        if ( isset( $query_vars['update_products'] ) ) {
            ...
            if ( ! current_user_can( 'dokan_view_tools_menu' ) ) {
                ...
                switch ( $query_vars['update_products'] ) {

                    case 'csv-import':
                        include_once( plugin_dir_path( __FILE__ ) . 'includes/import/class-wc-product-csv-importer.php' );
                        include_once( plugin_dir_path( __FILE__ ) . 'includes/importers/class-wc-product-csv-importer-controller.php' );

                        $importer = new Custom_WC_Product_CSV_Importer_Controller();
                        $importer->dispatch();
                        break;
    ...
    public function do_ajax_product_import() {
        ...
        $importer         = Custom_WC_Product_CSV_Importer_Controller::get_importer( $file, $params );
        ...
        if ( 100 === $percent_complete ) {
            ...
            wp_send_json_success( array(
                'position'   => 'done',
                'percentage' => 100,
                'url'        => dokan_get_navigation_url() . 'update_products/csv-import?step=done',
...
//            check_ajax_referer( 'wc-product-export', 'security' );
        ...
        if ( 100 === $exporter->get_percent_complete() ) {
            wp_send_json_success( array(
                'step'       => 'done',
                'percentage' => 100,
                'url'        => add_query_arg( array( 'nonce' => wp_create_nonce( 'product-csv' ), 'action' => 'download_product_csv' ), dokan_get_navigation_url( 'update_products/csv-export/' ) . '?product_exporter' ),
            ) );
        } else {
            wp_send_json_success( array(
                'step'       => ++$step,
    ...
    public function render_import_export_button() {
        ...
        <?php if ( current_user_can( 'dokan_import_product' ) ): ?>
            <a href="<?php echo dokan_get_navigation_url( 'update_products/csv-import' ) ?>" class="dokan-btn dokan-btn-theme">
                <?php _e( 'Import', 'dokan' ) ?>
            </a>
        <?php endif ?>
        <?php if ( current_user_can( 'dokan_export_product' ) && apply_filters( 'dokan_csv_export_enabled', true ) ) { ?>
            <a href="<?php echo dokan_get_navigation_url( 'update_products/csv-export' ) ?>" class="dokan-btn dokan-btn-theme">
                <?php _e( 'Export', 'dokan' ) ?>
            </a>
        <?php } ?>
        <?php
    ...
    function dashboard_active_menu( $active_menu ) {
        if ( $active_menu == 'update_products/csv-import' || $active_menu == 'update_products/csv-export' ) {
            $active_menu = 'update_products';
        }
        return $active_menu;
    }
    ...
    public function bind_global_ajaxurl() {
        ...
        if ( isset( $wp->query_vars['update_products'] ) ) {
            ?>
                <script>
...
$dokan_pi = Custom_Dokan_Product_Importer::init();

//dokan_register_activation_hook( __FILE__, array( 'Custom_Dokan_Product_Importer', 'activate' ) );
